...
const CheckValue = () => {
  if (
    document.getElementById("re-password-change").value.length < 6 ||
    document.getElementById("password-change").value.length < 6 ||
    document.getElementById("old-password-change").value.length < 6
  )
    AlertWarning("Mật khẩu phải nhiều hơn 6 kí tự");
  else if (
    document.getElementById("re-password-change").value !=
  ...
  return (
    <>
      ...
      <div className="change-pw-main">
        ...
        <button className="btn-change-pw" onClick={CheckValue}>
          Cập nhật
        </button>
      </div>
    </>
