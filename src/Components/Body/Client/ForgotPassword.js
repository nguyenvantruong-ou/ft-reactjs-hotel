...
const ForgotPassword = () => {
  ...
  const SendCode = () => {
    if (document.getElementById("email-fg").value.length < 6)
      ...
      fetch(URL + "Account/auth/code", {
        ...
        body: JSON.stringify({
          Email: document.getElementById("email-fg").value,
        }),
      })
        ...
        .then((results) => {
          if (results.code == 200) {
            ...
            localStorage.setItem(
              "Email",
              document.getElementById("email-fg").value
            );
  ...
  const ComfirmCodeFG = () => {
    if (document.getElementById("code-confirm-fg").value.length == 6) {
      fetch(URL + "Account/auth/confirm", {
        ...
        body: JSON.stringify({
          Code: document.getElementById("code-confirm-fg").value,
  ...
  const ResetPw = () => {
    if (
      document.getElementById("password-fg").value.length < 5 ||
      document.getElementById("password-again-fg").value.length < 5
    )
      AlertWarning("Mật khẩu phải trên 6 kí tự!");
    else if (
      document.getElementById("password-fg").value !=
      document.getElementById("password-again-fg").value
    )
      AlertWarning("Mật khẩu không khớp");
    else {
      fetch(URL + "Account/auth/password-update", {
        method: "POST", // or 'PUT'
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <>
      ...
      <div
        id="send-code-fg"
        style={{ marginLeft: "41%", marginTop: "70px" }}
        className="input-css"
      >
        <label>
          Email của bạn: <br />
      ...
      <div
        id="reset-pw-fg"
        style={{ marginLeft: "41%", marginTop: "30px", display: "none" }}
      >
        <div>
          <label>
            Mật khẩu: <br />
        ...
        <div>
          <button className="button-reset-pw" onClick={ResetPw}>
            Xác nhận
          </button>
        </div>
      </div>
